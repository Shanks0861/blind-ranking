...
class LobbyService {
  ...
  Future<Lobby> createLobby({
    ...
    final lobby = Lobby(
      ...
      code: code,
      hostId: hostId,
      listSize: ListSize.top10,
      status: LobbyStatus.waiting,
      createdAt: DateTime.now(),
    );
    await ref.set(lobby.toMap());
    await _db.collection('lobby_players').add({
      'lobby_id': ref.id,
      'user_id': hostId,
  ...
  Future<Lobby> joinLobby({
    ...
    final existing = await _db
        ...
        .where('lobby_id', isEqualTo: lobby.id)
  ...
  Future<void> updateLobbySettings({
    ...
    if (updates.isNotEmpty) {
      await _db.collection('lobbies').doc(lobbyId).update(updates);
    }
  }
  ...
  // Polling statt Firestore Streams (vermeidet WebChannel CORS Fehler)
  Stream<List<LobbyPlayer>> watchPlayers(String lobbyId) {
    return Stream.periodic(const Duration(seconds: 2)).asyncMap((_) async {
      final snap = await _db
          ...
          .get();
      return snap.docs
          .map((d) => LobbyPlayer.fromMap(d.id, d.data()))
          .toList();
    });
  }
